...
int libp2p_identify_message_decode(const uint8_t *buf, size_t len,
                                   libp2p_identify_t **out_msg)
{
    if (!buf || !out_msg)
        return -1;
    ...
    while (off < len) {
        ...
        if (unsigned_varint_decode(buf + off, len - off, &tag, &sz) != UNSIGNED_VARINT_OK ||
            !varint_is_minimal(tag, sz))
            goto fail;
        off += sz;
        if (unsigned_varint_decode(buf + off, len - off, &flen, &sz) != UNSIGNED_VARINT_OK ||
            flen > len - off - sz || !varint_is_minimal(flen, sz))
            goto fail;
        off += sz;
        ...
        case IDENTIFY_PUBLIC_KEY_TAG:
            ...
            if (!msg->public_key) goto fail;
            memcpy(msg->public_key, field, flen);
            msg->public_key_len = (size_t)flen;
            break;
        case IDENTIFY_LISTEN_ADDRS_TAG: {
            uint8_t **addrs = realloc(msg->listen_addrs,
                                     (msg->num_listen_addrs + 1) * sizeof(uint8_t*));
            size_t *lens = realloc(msg->listen_addrs_lens,
                                   (msg->num_listen_addrs + 1) * sizeof(size_t));
            if (!addrs || !lens) { free(addrs); free(lens); goto fail; }
        ...
        case IDENTIFY_PROTOCOLS_TAG: {
            char **protos = realloc(msg->protocols,
                                    (msg->num_protocols + 1) * sizeof(char*));
            if (!protos) goto fail;
        ...
        case IDENTIFY_OBSERVED_ADDR_TAG:
            ...
            if (!msg->observed_addr) goto fail;
        ...
        case IDENTIFY_PROTOCOL_VERSION_TAG:
            ...
            if (!msg->protocol_version) goto fail;
        ...
        case IDENTIFY_AGENT_VERSION_TAG:
            ...
            if (!msg->agent_version) goto fail;
...
int libp2p_identify_message_encode(const libp2p_identify_t *msg,
...
static int identify_protocol_handler(libp2p_stream_t *stream, void *user_data)
{
    ...
    if (libp2p_identify_message_encode(&response, &encoded_data, &encoded_len) != 0) {
        ...
        if (response.protocol_version) free(response.protocol_version);
        if (response.agent_version) free(response.agent_version);
        if (response.public_key) free(response.public_key);
        if (response.listen_addrs) free(response.listen_addrs);
        if (response.observed_addr) free(response.observed_addr);
        if (response.protocols) free(response.protocols);
        return -1;
    }
...
int libp2p_identify_register_handler(libp2p_protocol_handler_registry_t *registry,
                                     libp2p_identify_request_handler_t request_handler,
                                     void *user_data)
{
    if (!registry) {
    ...
    int result = libp2p_register_protocol_handler(registry, LIBP2P_IDENTIFY_PROTO_ID,
                                                  identify_protocol_handler, ctx);
...
int libp2p_identify_send_request_with_context(libp2p_protocol_handler_ctx_t *handler_ctx,
                                              libp2p_identify_response_handler_t response_handler,
                                              void *user_data)
{
    ...
    int result = libp2p_protocol_open_stream_with_context(
        (libp2p_mplex_ctx_t *)handler_ctx->muxer_ctx,
        handler_ctx->uconn,
    ...
    int msg_len = recv_length_prefixed_message(mx, stream->stream_id,
                                              stream->initiator, (char *)response_buf, sizeof(response_buf));
    if (msg_len <= 0) {
