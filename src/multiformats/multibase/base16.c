...
/* Base16 character and Unicode value */
#define BASE16_CHARACTER 'f'
#define BASE16_UNICODE 0x0066
...
static const uint8_t hex_values_table[256] = {
    ...
    0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
    0x80,
    0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,0x80,0x80,
    0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
    0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
    0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
    0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
...
/*
 * Extremely optimized Base16 (hexadecimal) encoding function.
 *
 * This function converts each input byte into two hex characters.
 * If the output pointer is 2-byte aligned, we write a full 16-bit word per byte.
 */
int base16_encode(const uint8_t *restrict data, size_t data_len, char *restrict out, size_t out_len)
{
    ...
    /* Use fast path if output is 2-byte aligned */
    ...
    {
        /* Unaligned output: write two bytes per input byte */
        for (size_t i = 0; i < data_len; i++)
        {
...
/*
 * Extremely optimized Base16 (hexadecimal) decoding function.
 *
 * This function decodes a hex string (which must have an even length) into binary.
 * It processes data in blocks (unrolled in groups of 16 hex characters = 8 bytes)
 * and uses a lookup table that marks invalid characters with the high bit set.
 */
int base16_decode(const char *restrict in, uint8_t *restrict out, size_t out_len)
{
    if (in == NULL || out == NULL)
        return MULTIBASE_ERR_NULL_POINTER;
    ...
    while (i < bulk_end) {
        ...
        if ((v0 | v1 | v2 | v3 | v4 | v5 | v6 | v7 |
             v8 | v9 | v10 | v11 | v12 | v13 | v14 | v15) & 0x80)
        {
