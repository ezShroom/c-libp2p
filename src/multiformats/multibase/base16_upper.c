 ...
 */
int base16_upper_encode(const uint8_t *restrict data, size_t data_len,
 ...
 */
int base16_upper_decode(const char *restrict in, size_t data_len,
                        uint8_t *restrict out, size_t out_len)
{
    ...
    uint8_t *out_ptr = out;
    size_t i = 0;
    /* Process in multiples of 8 bytes for speed. */
    size_t bulk_end = required_out_len - (required_out_len % 8);

    /* Bulk decode loop: decode 16 hex chars -> 8 bytes. */
    while (i < bulk_end)
    {
