...
int main(void)
{
    ...
    {
        int ret_encode = multibase_base16_encode(input_data, data_len, encoded, encoded_buffer_size);
        if (ret_encode < 0)
        {
        ...
        int ret_decode = multibase_base16_decode(encoded, ret_encode, decoded, data_len);
        if (ret_decode < 0)
        {
    ...
    printf("=== Benchmark Results for base16 ===\n");
