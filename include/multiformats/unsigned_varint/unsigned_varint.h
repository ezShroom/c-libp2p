 ...
 *       valid 63-bit number.
 */
mf_varint_err_t mf_uvarint_encode(uint64_t value, uint8_t *out, size_t out_size,
                                size_t *written);
 ...
 *         MF_VARINT_ERR_TOO_LONG if more than 9 bytes are required or if
 ...
 */
mf_varint_err_t mf_uvarint_decode(const uint8_t *in, size_t in_size, uint64_t *value,
                                size_t *read);
