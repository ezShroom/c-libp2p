#include <ctype.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>

#include "peer_id/peer_id.h"
#include "peer_id/peer_id_secp256k1.h"
#include "peer_id/peer_id_ed25519.h"
#include "peer_id/peer_id_ecdsa.h"
#include "peer_id/peer_id_rsa.h"

/* Helper function to print test results. */
static void print_standard(const char *test_name, const char *details, int passed)
{
    if (passed)
    {
        printf("TEST: %-50s | PASS\n", test_name);
    }
    else
    {
        printf("TEST: %-50s | FAIL: %s\n", test_name, details);
    }
}

/* Helper function: convert a hex string to a byte array.
   The caller must free the returned array.
*/
static uint8_t *hex_to_bytes(const char *hex, size_t *out_len)
{
    size_t hex_len = strlen(hex);
    if (hex_len % 2 != 0)
    {
        return NULL;
    }
    size_t bytes_len = hex_len / 2;
    uint8_t *bytes = malloc(bytes_len);
    if (!bytes)
    {
        return NULL;
    }
    for (size_t i = 0; i < bytes_len; i++)
    {
        char byte_str[3] = { hex[i * 2], hex[i * 2 + 1], '\0' };
        bytes[i] = (uint8_t)strtol(byte_str, NULL, 16);
    }
    *out_len = bytes_len;
    return bytes;
}

/* Test vectors (hex-encoded) for secp256k1 keys from the spec */
#define SECP256K1_PUBLIC_HEX "08021221037777e994e452c21604f91de093ce415f5432f701dd8cd1a7a6fea0e630bfca99"
#define SECP256K1_PRIVATE_HEX "0802122053DADF1D5A164D6B4ACDB15E24AA4C5B1D3461BDBD42ABEDB0A4404D56CED8FB"

/* Test vectors (hex-encoded) for RSA keys from the spec */
#define RSA_PRIVATE_HEX ""
#define RSA_PUBLIC_HEX  ""

/* Test vectors (hex-encoded) for ED25519 keys from the spec */
#define ED25519_PRIVATE_HEX "080112407e0830617c4a7de83925dfb2694556b12936c477a0e1feb2e148ec9da60fee7d1ed1e8fae2c4a144b8be8fd4b47bf3d3b34b871c3cacf6010f0e42d474fce27e"
#define ED25519_PUBLIC_HEX  "080112201ed1e8fae2c4a144b8be8fd4b47bf3d3b34b871c3cacf6010f0e42d474fce27e"

/* New test vectors (hex-encoded) for ECDSA keys from the spec */
#define ECDSA_PRIVATE_HEX "08031279307702010104203E5B1FE9712E6C314942A750BD67485DE3C1EFE85B1BFB520AE8F9AE3DFA4A4CA00A06082A8648CE3D030107A14403420004DE3D300FA36AE0E8F5D530899D83ABAB44ABF3161F162A4BC901D8E6ECDA020E8B6D5F8DA30525E71D6851510C098E5C47C646A597FB4DCEC034E9F77C409E62"
#define ECDSA_PUBLIC_HEX  "0803125b3059301306072a8648ce3d020106082a8648ce3d03010703420004de3d300fa36ae0e8f5d530899d83abab44abf3161f162a4bc901d8e6ecda020e8b6d5f8da30525e71d6851510c098e5c47c646a597fb4dcec034e9f77c409e62"

int main(void)
{
    int failures = 0;
    peer_id_error_t err;
    int ret;

    /***********************
     * Test 1: Create peer ID from a secp256k1 public key
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *pubkey = hex_to_bytes(SECP256K1_PUBLIC_HEX, &pubkey_len);
        if (!pubkey)
        {
            fprintf(stderr, "Failed to convert secp256k1 public hex to bytes\n");
            exit(EXIT_FAILURE);
        }

        peer_id_t pid_pub = {0};
        err = peer_id_create_from_public_key(pubkey, pubkey_len, &pid_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (secp256k1)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (secp256k1)", "", 1);
        }
        free(pubkey);

        /* Test peer_id_to_string for legacy (base58btc) format. */
        char legacy_str[256];
        ret = peer_id_to_string(&pid_pub, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (secp256k1)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "Legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (secp256k1)", details, 1);
        }

        /* Test peer_id_to_string for CIDv1 multibase format. */
        char cid_str[256];
        ret = peer_id_to_string(&pid_pub, PEER_ID_FMT_MULTIBASE_CIDv1, cid_str, sizeof(cid_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(CIDv1) (secp256k1)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "CIDv1 string: %s", cid_str);
            print_standard("peer_id_to_string(CIDv1) (secp256k1)", details, 1);
        }

        peer_id_destroy(&pid_pub);
    }

    /***********************
     * Test 2: Create peer ID from a secp256k1 private key
     ***********************/
    {
        size_t privkey_len = 0;
        uint8_t *privkey = hex_to_bytes(SECP256K1_PRIVATE_HEX, &privkey_len);
        if (!privkey)
        {
            fprintf(stderr, "Failed to convert secp256k1 private hex to bytes\n");
            exit(EXIT_FAILURE);
        }

        peer_id_t pid_priv = {0};
        err = peer_id_create_from_private_key(privkey, privkey_len, &pid_priv);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_private_key() (secp256k1)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_private_key() (secp256k1)", "", 1);
        }
        free(privkey);

        /* Also create peer ID from public key and compare. */
        size_t pubkey_len = 0;
        uint8_t *pubkey = hex_to_bytes(SECP256K1_PUBLIC_HEX, &pubkey_len);
        if (!pubkey)
        {
            fprintf(stderr, "Failed to convert secp256k1 public hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_pub = {0};
        err = peer_id_create_from_public_key(pubkey, pubkey_len, &pid_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (secp256k1)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (secp256k1)", "", 1);
        }
        free(pubkey);

        int eq = peer_id_equals(&pid_priv, &pid_pub);
        if (eq != 1)
        {
            print_standard("peer_id_equals() (secp256k1)", "Peer IDs do not match", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals() (secp256k1)", "", 1);
        }
        peer_id_destroy(&pid_priv);
        peer_id_destroy(&pid_pub);
    }

    /***********************
     * Test 3: Create peer ID from string representation (secp256k1)
     ***********************/
    {
        /* Legacy (raw base58btc) test */
        size_t pubkey_len = 0;
        uint8_t *pubkey = hex_to_bytes(SECP256K1_PUBLIC_HEX, &pubkey_len);
        if (!pubkey)
        {
            fprintf(stderr, "Failed to convert secp256k1 public hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid = {0};
        err = peer_id_create_from_public_key(pubkey, pubkey_len, &pid);
        free(pubkey);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (secp256k1)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (secp256k1)", "", 1);
        }

        char legacy_str[256];
        ret = peer_id_to_string(&pid, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (secp256k1)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "Legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (secp256k1)", details, 1);
        }

        peer_id_t pid_from_str = {0};
        err = peer_id_create_from_string(legacy_str, &pid_from_str);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_string(legacy) (secp256k1)", "Decoding failed", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_string(legacy) (secp256k1)", "", 1);
        }

        if (peer_id_equals(&pid, &pid_from_str) != 1)
        {
            print_standard("peer_id_equals(legacy) (secp256k1)", "Decoded peer ID does not match original", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals(legacy) (secp256k1)", "", 1);
        }
        peer_id_destroy(&pid);
        peer_id_destroy(&pid_from_str);
    }

    /***********************
     * Test 4: Create and compare RSA peer IDs (public and private)
     ***********************/
    {
        /* RSA public key test */
        size_t pubkey_len = 0;
        uint8_t *rsa_pubkey = hex_to_bytes(RSA_PUBLIC_HEX, &pubkey_len);
        if (!rsa_pubkey)
        {
            fprintf(stderr, "Failed to convert RSA public hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_rsa_pub = {0};
        err = peer_id_create_from_public_key(rsa_pubkey, pubkey_len, &pid_rsa_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (RSA)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (RSA)", "", 1);
        }
        free(rsa_pubkey);

        char legacy_str[256];
        ret = peer_id_to_string(&pid_rsa_pub, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (RSA)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "RSA Legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (RSA)", details, 1);
        }
        peer_id_destroy(&pid_rsa_pub);
    }

    {
        /* RSA private key test: generate peer ID from RSA private key and compare
           to the one derived from the corresponding RSA public key */
        size_t privkey_len = 0;
        uint8_t *rsa_privkey = hex_to_bytes(RSA_PRIVATE_HEX, &privkey_len);
        if (!rsa_privkey)
        {
            fprintf(stderr, "Failed to convert RSA private hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_rsa_priv = {0};
        err = peer_id_create_from_private_key(rsa_privkey, privkey_len, &pid_rsa_priv);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_private_key() (RSA)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_private_key() (RSA)", "", 1);
        }
        free(rsa_privkey);

        size_t pubkey_len = 0;
        uint8_t *rsa_pubkey = hex_to_bytes(RSA_PUBLIC_HEX, &pubkey_len);
        if (!rsa_pubkey)
        {
            fprintf(stderr, "Failed to convert RSA public hex to bytes (for RSA private test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_rsa_pub = {0};
        err = peer_id_create_from_public_key(rsa_pubkey, pubkey_len, &pid_rsa_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (RSA)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (RSA)", "", 1);
        }
        free(rsa_pubkey);

        int eq = peer_id_equals(&pid_rsa_priv, &pid_rsa_pub);
        if (eq != 1)
        {
            print_standard("peer_id_equals() (RSA)", "Peer IDs do not match", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals() (RSA)", "", 1);
        }
        peer_id_destroy(&pid_rsa_priv);
        peer_id_destroy(&pid_rsa_pub);
    }

    /***********************
     * Test 5: Test peer_id_equals with different peer IDs (secp256k1)
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *pubkey = hex_to_bytes(SECP256K1_PUBLIC_HEX, &pubkey_len);
        if (!pubkey)
        {
            fprintf(stderr, "Failed to convert secp256k1 public hex to bytes (equals test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid1 = {0}, pid2 = {0};
        err = peer_id_create_from_public_key(pubkey, pubkey_len, &pid1);
        err = peer_id_create_from_public_key(pubkey, pubkey_len, &pid2);
        free(pubkey);

        if (peer_id_equals(&pid1, &pid2) != 1)
        {
            print_standard("peer_id_equals(same IDs)", "Peer IDs should be equal", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals(same IDs)", "", 1);
        }

        /* Modify one byte in pid2 and ensure they are no longer equal */
        if (pid2.size > 0)
        {
            pid2.bytes[0] ^= 0xFF;
        }
        if (peer_id_equals(&pid1, &pid2) != 0)
        {
            print_standard("peer_id_equals(different IDs)", "Peer IDs should not be equal", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals(different IDs)", "", 1);
        }
        peer_id_destroy(&pid1);
        peer_id_destroy(&pid2);
    }

    /***********************
     * Test 6: Test peer_id_destroy
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *pubkey = hex_to_bytes(SECP256K1_PUBLIC_HEX, &pubkey_len);
        if (!pubkey)
        {
            fprintf(stderr, "Failed to convert secp256k1 public hex to bytes (destroy test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid = {0};
        err = peer_id_create_from_public_key(pubkey, pubkey_len, &pid);
        free(pubkey);
        peer_id_destroy(&pid);
        if (pid.bytes != NULL || pid.size != 0)
        {
            print_standard("peer_id_destroy()", "Peer ID not properly destroyed", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_destroy()", "", 1);
        }
    }

    /***********************
     * Test 7: Create peer ID from an ED25519 public key
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *ed_pubkey = hex_to_bytes(ED25519_PUBLIC_HEX, &pubkey_len);
        if (!ed_pubkey)
        {
            fprintf(stderr, "Failed to convert ED25519 public hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_ed_pub = {0};
        err = peer_id_create_from_public_key(ed_pubkey, pubkey_len, &pid_ed_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (ED25519)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (ED25519)", "", 1);
        }
        free(ed_pubkey);

        char legacy_str[256];
        ret = peer_id_to_string(&pid_ed_pub, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (ED25519)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "ED25519 legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (ED25519)", details, 1);
        }

        char cid_str[256];
        ret = peer_id_to_string(&pid_ed_pub, PEER_ID_FMT_MULTIBASE_CIDv1, cid_str, sizeof(cid_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(CIDv1) (ED25519)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "ED25519 CIDv1 string: %s", cid_str);
            print_standard("peer_id_to_string(CIDv1) (ED25519)", details, 1);
        }

        peer_id_destroy(&pid_ed_pub);
    }

    /***********************
     * Test 8: Create peer ID from an ED25519 private key and compare with public key
     ***********************/
    {
        size_t privkey_len = 0;
        uint8_t *ed_privkey = hex_to_bytes(ED25519_PRIVATE_HEX, &privkey_len);
        if (!ed_privkey)
        {
            fprintf(stderr, "Failed to convert ED25519 private hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_ed_priv = {0};
        err = peer_id_create_from_private_key(ed_privkey, privkey_len, &pid_ed_priv);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_private_key() (ED25519)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_private_key() (ED25519)", "", 1);
        }
        free(ed_privkey);

        size_t pubkey_len = 0;
        uint8_t *ed_pubkey = hex_to_bytes(ED25519_PUBLIC_HEX, &pubkey_len);
        if (!ed_pubkey)
        {
            fprintf(stderr, "Failed to convert ED25519 public hex to bytes (for private key test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_ed_pub = {0};
        err = peer_id_create_from_public_key(ed_pubkey, pubkey_len, &pid_ed_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (ED25519 for private key test)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (ED25519 for private key test)", "", 1);
        }
        free(ed_pubkey);

        int eq = peer_id_equals(&pid_ed_priv, &pid_ed_pub);
        if (eq != 1)
        {
            print_standard("peer_id_equals(ED25519 private vs public)", "Peer IDs do not match", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals(ED25519 private vs public)", "", 1);
        }
        peer_id_destroy(&pid_ed_priv);
        peer_id_destroy(&pid_ed_pub);
    }

    /***********************
     * Test 9: Create peer ID from string representation (ED25519)
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *ed_pubkey = hex_to_bytes(ED25519_PUBLIC_HEX, &pubkey_len);
        if (!ed_pubkey)
        {
            fprintf(stderr, "Failed to convert ED25519 public hex to bytes (string test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid = {0};
        err = peer_id_create_from_public_key(ed_pubkey, pubkey_len, &pid);
        free(ed_pubkey);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (ED25519)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (ED25519)", "", 1);
        }

        char legacy_str[256];
        ret = peer_id_to_string(&pid, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (ED25519)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "ED25519 legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (ED25519)", details, 1);
        }

        peer_id_t pid_from_str = {0};
        err = peer_id_create_from_string(legacy_str, &pid_from_str);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_string(legacy) (ED25519)", "Decoding failed", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_string(legacy) (ED25519)", "", 1);
        }

        if (peer_id_equals(&pid, &pid_from_str) != 1)
        {
            print_standard("peer_id_equals(legacy) (ED25519)", "Decoded peer ID does not match original", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals(legacy) (ED25519)", "", 1);
        }
        peer_id_destroy(&pid);
        peer_id_destroy(&pid_from_str);
    }

    /***********************
     * Test 10: Create peer ID from an ECDSA public key
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *ecdsa_pubkey = hex_to_bytes(ECDSA_PUBLIC_HEX, &pubkey_len);
        if (!ecdsa_pubkey)
        {
            fprintf(stderr, "Failed to convert ECDSA public hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_ecdsa_pub = {0};
        err = peer_id_create_from_public_key(ecdsa_pubkey, pubkey_len, &pid_ecdsa_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (ECDSA)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (ECDSA)", "", 1);
        }
        free(ecdsa_pubkey);

        char legacy_str[256];
        ret = peer_id_to_string(&pid_ecdsa_pub, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (ECDSA)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "ECDSA legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (ECDSA)", details, 1);
        }

        char cid_str[256];
        ret = peer_id_to_string(&pid_ecdsa_pub, PEER_ID_FMT_MULTIBASE_CIDv1, cid_str, sizeof(cid_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(CIDv1) (ECDSA)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "ECDSA CIDv1 string: %s", cid_str);
            print_standard("peer_id_to_string(CIDv1) (ECDSA)", details, 1);
        }

        peer_id_destroy(&pid_ecdsa_pub);
    }

    /***********************
     * Test 11: Create peer ID from an ECDSA private key and compare with public key
     ***********************/
    {
        size_t privkey_len = 0;
        uint8_t *ecdsa_privkey = hex_to_bytes(ECDSA_PRIVATE_HEX, &privkey_len);
        if (!ecdsa_privkey)
        {
            fprintf(stderr, "Failed to convert ECDSA private hex to bytes\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_ecdsa_priv = {0};
        err = peer_id_create_from_private_key(ecdsa_privkey, privkey_len, &pid_ecdsa_priv);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_private_key() (ECDSA)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_private_key() (ECDSA)", "", 1);
        }
        free(ecdsa_privkey);

        size_t pubkey_len = 0;
        uint8_t *ecdsa_pubkey = hex_to_bytes(ECDSA_PUBLIC_HEX, &pubkey_len);
        if (!ecdsa_pubkey)
        {
            fprintf(stderr, "Failed to convert ECDSA public hex to bytes (for private key test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid_ecdsa_pub = {0};
        err = peer_id_create_from_public_key(ecdsa_pubkey, pubkey_len, &pid_ecdsa_pub);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (ECDSA for private key test)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (ECDSA for private key test)", "", 1);
        }
        free(ecdsa_pubkey);

        int eq = peer_id_equals(&pid_ecdsa_priv, &pid_ecdsa_pub);
        if (eq != 1)
        {
            print_standard("peer_id_equals() (ECDSA)", "Peer IDs do not match", 0);
            fprintf(stderr, "ECDSA Private Key ID: ");
            for (size_t i = 0; i < pid_ecdsa_priv.size; i++)
            {
                fprintf(stderr, "%02x", pid_ecdsa_priv.bytes[i]);
            }
            fprintf(stderr, "\nECDSA Public Key ID: ");
            for (size_t i = 0; i < pid_ecdsa_pub.size; i++)
            {
                fprintf(stderr, "%02x", pid_ecdsa_pub.bytes[i]);
            }
            fprintf(stderr, "\n");
            failures++;
        }
        else
        {
            print_standard("peer_id_equals() (ECDSA)", "", 1);
        }
        peer_id_destroy(&pid_ecdsa_priv);
        peer_id_destroy(&pid_ecdsa_pub);
    }

    /***********************
     * Test 12: Create peer ID from string representation (ECDSA)
     ***********************/
    {
        size_t pubkey_len = 0;
        uint8_t *ecdsa_pubkey = hex_to_bytes(ECDSA_PUBLIC_HEX, &pubkey_len);
        if (!ecdsa_pubkey)
        {
            fprintf(stderr, "Failed to convert ECDSA public hex to bytes (string test)\n");
            exit(EXIT_FAILURE);
        }
        peer_id_t pid = {0};
        err = peer_id_create_from_public_key(ecdsa_pubkey, pubkey_len, &pid);
        free(ecdsa_pubkey);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_public_key() (ECDSA)", "Returned error", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_public_key() (ECDSA)", "", 1);
        }

        char legacy_str[256];
        ret = peer_id_to_string(&pid, PEER_ID_FMT_BASE58_LEGACY, legacy_str, sizeof(legacy_str));
        if (ret < 0)
        {
            print_standard("peer_id_to_string(legacy) (ECDSA)", "Encoding failed", 0);
            failures++;
        }
        else
        {
            char details[256];
            sprintf(details, "ECDSA legacy string: %s", legacy_str);
            print_standard("peer_id_to_string(legacy) (ECDSA)", details, 1);
        }

        peer_id_t pid_from_str = {0};
        err = peer_id_create_from_string(legacy_str, &pid_from_str);
        if (err != PEER_ID_SUCCESS)
        {
            print_standard("peer_id_create_from_string(legacy) (ECDSA)", "Decoding failed", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_create_from_string(legacy) (ECDSA)", "", 1);
        }

        if (peer_id_equals(&pid, &pid_from_str) != 1)
        {
            print_standard("peer_id_equals(legacy) (ECDSA)", "Decoded peer ID does not match original", 0);
            failures++;
        }
        else
        {
            print_standard("peer_id_equals(legacy) (ECDSA)", "", 1);
        }
        peer_id_destroy(&pid);
        peer_id_destroy(&pid_from_str);
    }

    if (failures)
    {
        printf("\nSome tests failed. Total failures: %d\n", failures);
        return EXIT_FAILURE;
    }
    else
    {
        printf("\nAll tests passed!\n");
        return EXIT_SUCCESS;
    }
}