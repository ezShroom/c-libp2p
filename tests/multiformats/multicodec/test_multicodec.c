...
int main(void)
{
    ...
    if (num_mappings != 591) {
        char details[256];
        sprintf(details,
                "Expected 591 total mappings, but found %d. Either update your multicodec_mappings.h with all entries, or adjust this check.",
                num_mappings);
    ...
    for (int i = 0; i < num_mappings; i++) {
        ...
        if (actual_code != expected_code) {
            char details[256];
            sprintf(details,
                    "returned 0x%llx, expected 0x%llx",
                    (unsigned long long)actual_code,
