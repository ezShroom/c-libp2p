...
static int test_encode_decode(uint64_t value, const char *expected_hex)
{
...
#ifdef _MSC_VER
        sprintf_s(details, sizeof(details), "Encoding error for value=%llu, err=%d",
    ...
    {
#ifdef _MSC_VER
        sprintf_s(details, sizeof(details), "Encoded hex mismatch: got %s, expected %s",
    ...
    {
#ifdef _MSC_VER
        sprintf_s(details, sizeof(details), "Decoding error for value=%llu, err=%d",
                  (unsigned long long)value, (int)err);
#else
...
static int test_decode_failure(const uint8_t *data, size_t data_len,
                               unsigned_varint_err_t expected_err, const char *test_name)
{
    uint64_t decoded_val = 0;
    size_t read = 0;
    ...
    {
#ifdef _MSC_VER
        sprintf_s(details, sizeof(details), "Expected error %d but got %d", (int)expected_err,
                  (int)err);
#else
...
static int test_encode_failure(uint64_t value, unsigned_varint_err_t expected_err,
                               const char *test_name)
{
    ...
    {
#ifdef _MSC_VER
        sprintf_s(details, sizeof(details), "Expected error %d for value %llu, but got %d",
                  (int)expected_err, (unsigned long long)value, (int)err);
#else
...
static int test_unsigned_varint_size(uint64_t value, size_t expected_size)
{
    char test_name[128];
#ifdef _MSC_VER
    sprintf_s(test_name, sizeof(test_name), "unsigned_varint_size for value=%llu",
...
int main(void)
{
...

    {
        ...
        failures +=
            test_decode_failure(non_minimal_for_1, sizeof(non_minimal_for_1),
                                UNSIGNED_VARINT_ERR_NOT_MINIMAL, "Non-minimal encoding for 1")
                ? 0
                : 1;
    }

    {
        ...
        failures += test_decode_failure(ten_bytes, sizeof(ten_bytes), UNSIGNED_VARINT_ERR_TOO_LONG,
                                        "10-byte sequence")
                        ? 0
                        : 1;
    }

    {
        ...
        failures += test_decode_failure(truncated, sizeof(truncated), UNSIGNED_VARINT_ERR_TOO_LONG,
    ...
    }
    {
        ...
        failures += test_decode_failure(truncated_larger, sizeof(truncated_larger),
                                        UNSIGNED_VARINT_ERR_TOO_LONG, "Truncated single 0xFF")
                        ? 0
                        : 1;
    }

    failures += test_encode_failure(0xFFFFFFFFFFFFFFFFULL, UNSIGNED_VARINT_ERR_VALUE_OVERFLOW,
                                    "Encoding >2^63-1")
                    ? 0
                    : 1;
    failures += test_encode_failure(0x8000000000000000ULL, UNSIGNED_VARINT_ERR_VALUE_OVERFLOW,
                                    "Encoding 2^63")
                    ? 0
                    : 1;

    {
        uint8_t empty[1] = {};
        failures +=
            test_decode_failure(empty, 0, UNSIGNED_VARINT_ERR_TOO_LONG, "Empty input decode") ? 0
                                                                                              : 1;
    }

    {
        ...
        failures += test_decode_failure(varint_2_63, sizeof(varint_2_63),
                                        UNSIGNED_VARINT_ERR_VALUE_OVERFLOW, "Decode 2^63")
                        ? 0
                        : 1;
    }
