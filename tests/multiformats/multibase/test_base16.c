...
int main(void)
{
    ...
    for (size_t i = 0; i < num_tests; i++)
    {
        ...
        int ret =
            multibase_base16_encode((const uint8_t *)tv.input, input_len, encoded, out_buf_size);
        char test_name[128];
        sprintf(test_name, "multibase_base16_encode(\"%s\")", tv.input);
        if (ret < 0)
        {
            char details[256];
        ...
        int ret_dec = multibase_base16_decode(encoded, ret, decoded, decode_buf_size);
        sprintf(test_name, "multibase_base16_decode(\"%s\")", encoded);
        if (ret_dec < 0)
        {
            char details[256];
