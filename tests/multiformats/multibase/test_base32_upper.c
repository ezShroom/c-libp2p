...
extern int base32_upper_encode(const uint8_t *data, size_t data_len, char *out, size_t out_len);
extern int base32_upper_decode(const char *in, uint8_t *out, size_t out_len);
...
int main(void)
{
    ...
    for (size_t i = 0; i < num_tests; i++)
    {
        ...
        int ret = base32_upper_encode((const uint8_t *)tv.input, input_len, encoded, out_buf_size - 1);
        char test_name[128];
        ...
        int ret_dec = base32_upper_decode(encoded, decoded, decode_buf_size);
